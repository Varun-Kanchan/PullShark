...
Inside the Colab notebook, you'll find a single configuration cell with interactive form fields:

| Variable            | Description                                      | Example Value           |
|---------------------|--------------------------------------------------|-------------------------|
| `GITHUB_USERNAME`   | Your GitHub handle                               | `"Shineii86"`           |
| `GITHUB_TOKEN`      | Personal Access Token (keep secret!)             | `"ghp_abc123..."`       |
| `REPO_NAME`         | Target repository (must exist under your account)| `"PullShark"`           |
| `NUM_PRS`           | Number of PRs to create (min 2 for achievement)  | `4`                     |
| `BASE_BRANCH`       | Branch to target (default: `main`)               | `"main"`                |
| `DELAY_SECONDS`     | Pause between PRs and merge retries              | `10`                    |
| `MAX_RETRIES`       | Merge attempts before giving up                  | `3`                     |
...
- Create and merge the specified number of pull requests
...
🎉 Merged PR #178 on attempt 1
⏸️ Pausing 10 seconds for GitHub to process the merge...
...
...
- **File Modified**: By default, the script updates or creates `README.md`. To change this, edit the line `repo.get_contents("README.md", ...)` in the notebook.
- **Repository**: Use any repo you own or have write access to — just update `REPO_NAME`.
